...
var ALL_STATUS_FIELDS = ['reservoir', 'battery', 'clock', 'status', 'signal', 'device'];
...
function init (ctx) {
  ...
  function updateSignal (type, prefs, result) {
    if (result.signal) {
      result.signal.label = 'RSSI';
      result.signal.display = result.signal.value + type;
      var warn = -80;
      var urgent = -90;

      if (result.signal.value < urgent) {
        result.signal.level = levels.URGENT;
        result.signal.message = 'URGENT: Pump RSSI Low';
      } else if (result.signal.value < warn) {
        result.signal.level = levels.WARN;
        result.signal.message = 'Warning, Pump RSSI Low';
      } else {
        result.signal.level = levels.NONE;
      }
    }
  }
  ...
  function buildMessage (result) {
    if (result.level > levels.NONE) {
      ...
      //if (result.signal) {
      //  message.push('Pump RSSI: ' + result.signal.display);
      //}
