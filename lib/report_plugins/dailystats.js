...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  $('<th style="background-color:#f55">' + translate('Low') + '<br />&nbsp;</th>').appendTo(thead);
  if (options.superLow < options.targetLow) {
    $('<th style="background-color:#fb0">' + translate('Lowish') + '<font size="-1"><br/><div style="float:left;">' + options.superLow.toFixed(1) + '</div></font></th>').appendTo(thead);
  }
  $('<th style="background-color:#8f8">' + translate('In Range') + '<font size="-1"><br/><div style="float:left;">' + options.targetLow.toFixed(1) + '</div><div style="float:right;">' + options.targetHigh.toFixed(1) + '</div></font></th>').appendTo(thead);
  if (options.targetHigh < options.superHigh) {
    $('<th style="background-color:#cf0">' + translate('Highish') + '<font size="-1"><br/><div style="float:right;">' + options.superHigh.toFixed(1) + '</div></font></th>').appendTo(thead);
  }
  $('<th style="background-color:#ff0">' + translate('High') + '<br />&nbsp;</th>').appendTo(thead);
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    $('<td class="tdborder">' + ss.standard_deviation(bgValues).toFixed(1) + '<br/>(' +
        Math.round(100 * ss.standard_deviation(bgValues) / average+0.5) + '%)</td>').appendTo(tr);
