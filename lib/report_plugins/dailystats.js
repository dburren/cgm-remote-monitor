...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    if (stats.normal - daysRecords.length == 0) {
      $('<td class="tdborder" style="background-color:#7df27d">' + ((100.0 * stats.normal) / daysRecords.length).toFixed(1) + '%</td>').appendTo(tr);
    } else {
      if (stats.normal + stats.highish - daysRecords.length == 0) {
        $('<td class="tdborder" style="background-color:#54c954">' + ((100.0 * stats.normal) / daysRecords.length).toFixed(1) + '%</td>').appendTo(tr);
      } else {
    ...
    if (options.targetHigh < options.superHigh) {
      if (stats.highish > 0 && stats.normal + stats.highish - daysRecords.length == 0) {
        $('<td class="tdborder" style="background-color:#54c954">' + ((100.0 * stats.highish) / daysRecords.length).toFixed(1) + '%</td>').appendTo(tr);
      } else {
