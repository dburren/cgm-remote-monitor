...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  $('<th style="background-color:#ff5454">' + translate('Low') + '<br />&nbsp;</th>').appendTo(thead);
  if (options.superLow < options.targetLow) {
    $('<th style="background-color:#ffbb98">' + translate('Lowish') + '<font size="-1"><br/><div style="float:left;">&nbsp;' + options.superLow + '</div></font></th>').appendTo(thead);
  }
  $('<th style="background-color:#7df27d">' + translate('In Range') + '<font size="-1"><br/><div style="float:left;">&nbsp;' + options.targetLow + '</div><div style="float:right;">' + options.targetHigh + '&nbsp;</div></font></th>').appendTo(thead);
  if (options.targetHigh < options.superHigh) {
    $('<th style="background-color:#54c954">' + translate('Highish') + '<font size="-1"><br/><div style="float:right;">' + options.superHigh + '&nbsp;</div></font></th>').appendTo(thead);
  }
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    $('<td class="tdborder" style="width:160px">' + report_plugins.utils.localeDate(day) + '</td>').appendTo(tr);
    var stat = ((100.0 * stats.lows) / daysRecords.length).toFixed(1) + '%';
    if (stat == "0.0%") {
    	stat = "0%";
    }
    $('<td class="tdborder">' + stat + '</td>').appendTo(tr);
    if (options.superLow < options.targetLow) {
      stat = ((100.0 * stats.lowish) / daysRecords.length).toFixed(1) + '%';
      if (stat == "0.0%") {
    	  stat = "0%";
      }
      $('<td class="tdborder">' + stat + '</td>').appendTo(tr);
    }
    stat = ((100.0 * stats.normal) / daysRecords.length).toFixed(1) + '%';
    if (stat == "0.0%") {
      stat = "0%";
    }
    if (stats.normal - daysRecords.length == 0) {
      $('<td class="tdborder" style="background-color:#7df27d">' + stat + '</td>').appendTo(tr);
    } else {
      if (stats.normal + stats.highish - daysRecords.length == 0) {
        $('<td class="tdborder" style="background-color:#54c954">' + stat + '</td>').appendTo(tr);
      } else {
        $('<td class="tdborder">' + stat + '</td>').appendTo(tr);
      }
    }
    if (options.targetHigh < options.superHigh) {
      stat = ((100.0 * stats.highish) / daysRecords.length).toFixed(1) + '%';
      if (stat == "0.0%") {
        stat = "0%";
      }
      if (stats.highish > 0 && stats.normal + stats.highish - daysRecords.length == 0) {
        $('<td class="tdborder" style="background-color:#54c954">' + stat + '</td>').appendTo(tr);
      } else {
        $('<td class="tdborder">' + stat + '</td>').appendTo(tr);
      }
    }
    stat = ((100.0 * stats.highs) / daysRecords.length).toFixed(1) + '%';
    if (stat == "0.0%") {
      stat = "0%";
    }
    $('<td class="tdborder">' + stat + '</td>').appendTo(tr);
