...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  $('<th style="background-color:#8f8">' + translate('In Range') + '<font size="-1"><br/>' + options.targetLow.toFixed(1) + '&nbsp;%nbsp;' + options.targetHigh.toFixed(1) + '</font></th>').appendTo(thead);
  if (options.targetHigh < options.superHigh) {
    $('<th style="background-color:#cf0">' + translate('Highish') + '<font size="-1"><br/>&nbsp;&nbsp;' + options.superHigh.toFixed(1) + '</font></th>').appendTo(thead);
  }
