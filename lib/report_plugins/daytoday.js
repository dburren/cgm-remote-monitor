...
daytoday.html = function html (client) {
  ...
  var ret =
    ...
    `<label><input type="checkbox" id="rp_optionspredicted" ${reportStorage.getValue('predicted') ? "checked" : ""}><span style="color:sienna;opacity:0.5">${translate('Forecasts')}</span></label>` +
...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (options.insulindistribution) {
    var html = '<br><br><b>' + translate('TDD average') + ':</b> ' + tddAverage.toFixed(2) + 'U&nbsp; ';
  ...
  function drawChart (day, data, options) {
    ...
    if (options.insulindistribution) {
      ...
      var table = $('<table>');
      $('<tr><td>' + translate('Bolus insulin:') + '</td><td align="right">' + bolusInsulin.toFixed(2) + 'U</td></tr>').appendTo(table);
      $('<tr><td><b>' + translate('Base basal insulin:') + '</b></td><td align="right">' + baseBasalInsulin.toFixed(2) + 'U</td></tr>').appendTo(table);
      if (positiveTemps > 0) {
        $('<tr><td>' + translate('Positive temp basal insulin:') + '</td><td align="right">' + positiveTemps.toFixed(2) + 'U</td></tr>').appendTo(table);
      }
      if (negativeTemps < 0) {
        $('<tr><td>' + translate('Negative temp basal insulin:') + '</td><td align="right">' + negativeTemps.toFixed(2) + 'U</td></tr>').appendTo(table);
      }
      var totalBasalInsulin = baseBasalInsulin + positiveTemps + negativeTemps;
      $('<tr><td>' + translate('Total basal insulin:') + '</td><td align="right">' + totalBasalInsulin.toFixed(2) + 'U</td></tr>').appendTo(table);
      var totalDailyInsulin = bolusInsulin + baseBasalInsulin + positiveTemps + negativeTemps;
      $('<tr><td><b>' + translate('Total daily insulin:') + '</b></td><td align="right">' + totalDailyInsulin.toFixed(2) + 'U</td></tr>').appendTo(table);
      ...
      var chartData = [
        ...
        , { label: translate('non-base Basal'), count: (totalDailyInsulin-baseBasalInsulin), pct: ((totalDailyInsulin-baseBasalInsulin) / totalDailyInsulin * 100).toFixed(0) }
      ];
