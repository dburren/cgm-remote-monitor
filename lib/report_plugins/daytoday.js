...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (options.insulindistribution) {
    var html = '<br><br><b>' + translate('TDD average') + ':</b> ' + tddAverage.toFixed(3) + 'U&nbsp; ';
  ...
  function drawChart (day, data, options) {
	...
	if (Math.round(displayval) == displayval) {
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
      ...
      } else if ((treatment.eventType === 'Glucose Check' || treatment.eventType === 'BG Check') && !treatment.duration && options.bgcheck) {
        ...
        context.append('text')
          ...
          .text(translate(client.careportal.resolveEventName(treatment.eventType)) + ' a');
      } else if (treatment.duration && options.othertreatments) {
    ...
    if (options.insulindistribution) {
      ...
      $('<tr><td><b>' + translate('Total daily insulin:') + '</b></td><td align="right">' + totalDailyInsulin.toFixed(3) + 'U</td></tr>').appendTo(table);
