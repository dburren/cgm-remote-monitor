...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'Normal') {
      if (titrcount > 1) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '">' + translate('TISR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalRange == 2) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '">' + translate('TITR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalRange == 3) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '">' + translate('TIR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
