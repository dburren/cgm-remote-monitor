...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<label for="rp_noting" class="translate">Standards (clean)</label>' +
    '&nbsp;' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var tar2 = (Number(result.SuperHigh.readingspct) + Number(result.High.readingspct)).toFixed(1);
  var tar2count = !result.SuperHigh.ignore + !result.High.ignore;

  var tar3 = Number(result.Elevated.readingspct) + Number(tar2);
  var tar3count = !result.Elevated.ignore + tar2count;
  var preg3 = Number(result.High.readingspct) + Number(result.Elevated.readingspct);
  var preg3count = !noGravid * (!result.Elevated.ignore + !result.High.ignore);

  var tar4 = Number(result.Up.readingspct) + Number(tar3);
  var tar4count = !result.Up.ignore + tar3count;

  var preg1 = Number(result.Normal.readingspct) + Number(result.Lowish.readingspct);
  var preg2 = result.Up.readingspct + preg1;

  var titr = Number(result.Normal.readingspct) + Number(result.Up.readingspct);
  var titrcount = !result.Normal.ignore + !result.Up.ignore;
  var tir = Number(titr) + Number(result.Elevated.readingspct);
  var tircount = titrcount + !result.Elevated.ignore;

  var tbr1 = Number(result.Low.readingspct) + Number(result.SuperLow.readingspct);
  var tbr1count = !result.Low.ignore + !result.SuperLow.ignore;

  var tbr2 = Number(tbr1) + Number(result.Lowish.readingspct);
  var tbr2count = !result.Lowish.ignore + tbr1count;

  var tbr3 = Number(result.Low.readingspct) + Number(result.Lowish.readingspct);
  ...
  var tar2 = Number(tar2).toFixed(1);
  var tar3 = Number(tar3).toFixed(1);
  var tar4 = Number(tar4).toFixed(1);
  var preg1 = Number(preg1).toFixed(1);
  var preg2 = Number(preg2).toFixed(1);
  var preg3 = Number(preg3).toFixed(1);
  var titr = Number(titr).toFixed(1);
  var tir = Number(tir).toFixed(1);
  var tbr1 = Number(tbr1).toFixed(1);
  var tbr2 = Number(tbr2).toFixed(1);
  var tbr3 = Number(tbr3).toFixed(1);
