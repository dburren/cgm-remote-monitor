...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var preg3count = !result.Elevated.ignore + !result.High.ignore;
  ...
  var hyperComponent = result.SuperHigh.readingspct + 0.5 * (result.Elevated.readingspct + result.High.readingspct);
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'High' && !custom && preg3count > 0) {
...
//  $('<tr><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td></tr>').appendTo(stabilitytable);
  ...
  if (!!custom) {
    $('<tr><th>' + translate('GRI') + '</th><th>' + translate('Hypo Component') + '</th><th>Hyper Component</th></tr>').appendTo(stabilitytable);
    $('<tr><td class="tdborder">' + Math.round(gri * 10) / 10 + '</td><td class="tdborder">' + Math.round(hypoComponent * 10) / 10 + '</td><td class="tdborder">' + Math.round(hyperComponent * 10) / 10 + '</td></tr>').appendTo(stabilitytable);
  }
