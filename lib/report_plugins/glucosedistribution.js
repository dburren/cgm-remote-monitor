  ...
  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    ...
    if (range == 'High') {
      ...
      if (options.superHigh != options.targetHigh) {
        $('<td class="tdborder">' + r.readingspct.toFixed(1) + '% (' + tar.toFixed(1) + '%)</td>').appendTo(tr);
      } else {
        $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      } 
    }
    ...
    if (range == 'Normal' || range == 'Lowish') {
      $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
    }
    if (range == 'Low') {
      if (options.superLow != options.targetLow) {
        $('<td class="tdborder">' + r.readingspct.toFixed(1) + '% (<strong>' + tbr.toFixed(1) + '%</strong>)</td>').appendTo(tr);
      } else {
        $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } 
    } 
