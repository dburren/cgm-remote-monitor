...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  let captured = glucose_data.length;
  if (captured > 288*days) {
    captured = 288*days;
  }
  var tr = $('<tr>');
  $('<td class="tdborder"><strong>' + translate('Overall') + ': </strong></td>').appendTo(tr);
  $('<td> </td>').appendTo(tr);
  $('<td class="tdborder">' + glucose_data.length + ' (' + (Math.round(1000*captured/(288*days))/10).toFixed(1) + '%)</td>').appendTo(tr);
