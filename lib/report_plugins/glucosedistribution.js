...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '  </h2>' +
    '<table><tr>' +
    '<td style="valign:middle;"><div id="glucosedistribution-overviewchart"></div></td>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var colors = ['#f88', '#fb0', '#8f8', '#cf8', '#cf0', '#ff8'];
  var tablecolors = {
    Low: '#f88'
    , Lowish: '#fb0'
    , Normal: '#8f8'
  ...
  ['Low', 'Lowish', 'Normal', 'Up', 'Elevated', 'High'].forEach(function(range) {
    result[range] = {};
    var r = result[range];
  ...
  result.Normal.readingspct = (100 - result.Low.readingspct - result.Lowish.readingspct - result.Up.readingspct - result.Elevated.readingspct - result.High.readingspct);

  var tar = result.Elevated.readingspct + result.High.readingspct;
  var tnr = result.Normal.readingspct + result.Up.readingspct;
  var tir = tnr + result.Elevated.readingspct;
  var tbr = result.Low.readingspct + result.Lowish.readingspct;

  ['High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    var tr = $('<tr>');
    var r = result[range];
    ...
    if (range == 'Up' && !!haveStrict) {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" rowspan="2"><strong>' + tnr.toFixed(1) + '%</strong></td>').appendTo(tr);
      if (extrarangecols > 0) {
        $('<td></td>').appendTo(tr);
      }
    } 
    ...
    if (range == 'Lowish') {
      ...
      $('<td class="tdborder" rowspan="2"><strong>' + tbr.toFixed(1) + '%</strong></td>').appendTo(tr);
    }
    if (range == 'Low') {
      $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      if (!!haveStrict) {
  ...
  if (glucose_data.length > 0) {
    ...
    //  $('<td class="tdborder"><center><strong>' + a1cmd + '%</strong><sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub></center></td>').appendTo(tr);
    //} else {
    //  $('<td class="tdborder"><center>Mean:&nbsp;' + a1cmd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub>' +
    //      '<br />Weighted:&nbsp;' + a1cwd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cwi + '<sub>IFCC</sub></center></td>').appendTo(tr);
    //}
    //if (gmim === gmiw) {
    //  $('<td class="tdborder"><center>' + gmim + '%</center></td>').appendTo(tr);
    //} else {
    //  $('<td class="tdborder"><center>Mean:&nbsp;' + gmim + '%<br />Weighted:&nbsp;' + gmiw + '%</center></td>').appendTo(tr);
    //}
    $('<td class="tdborder"><center><strong>' + a1cwd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cwi + '<sub>IFCC</sub></strong></center></td>').appendTo(tr);
    $('<td class="tdborder"><center>' + gmiw + '%</center></td>').appendTo(tr);
  } else {
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
