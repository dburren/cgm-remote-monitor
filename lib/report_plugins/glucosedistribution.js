...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'SuperLow') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (!custom && !noGravid) {
        $('<td></td>').appendTo(tr);
      } 
    } 
  ...
  if (glucose_data.length > 0) {
    ...
    if (!extra || (a1cmd === a1cwd && a1cmi === a1cwi && gmim == gmiw)) {
      ...
      $('<td class="tdborderl">' + a1cmd + '<sub>&nbsp;%</sub>&nbsp;(' + mina1cmd + '<sub>%nbsp;%</sub>) ' + a1cmi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mina1cmi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmim + '<sub>&nbsp;%</sub>&nbsp;(' + mingmim + '<sub>&nbsp;%</sub>) ' + gmimi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mingmimi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
