...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var preg3 = (Number(result.High.readingspct) + Number(result.Elevated.readingspct)0).toFixed(1);
  ...
  var tir = $(titr + result.Elevated.readingspct + 0).tofixed(1);
  var tircount = titrcount + !result.Elevated.ignore;

  var tbr1 = $(result.Low.readingspct + result.SuperLow.readingspct + 0).toFixed(1);
  var tbr1count = !result.Low.ignore + !result.SuperLow.ignore;

  var tbr2 = $(tbr1 + result.Lowish.readingspct + 0).toFixed(1);
  var tbr2count = !result.Lowish.ignore + tbr1count;

  var tbr3 = $(result.Low.readingspct + result.Lowish.readingspct + 0).toFixed(1);
