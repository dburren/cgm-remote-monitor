...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('here') +
    '</a>.<br/><br/>' +
    'Symmetric Risk Profile (low and high) indices.</div><br/><br/>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var srp1_hgbi_total = 0;
  var srp1_lgbi_total = 0;
  ...
  for (i = 0; i <= glucose_data.length - 2; i++) {
    ...
    // constants for mg/dL
    var f = ((Math.log(entry.bgValue) ** 1.084) - 5.381) * 1.509;
    var r = 10 * (f ** 2);
    if (f > 0) {
    	srp0_hgbi_total += r;
    }
    ...
    // constants for mg/dL
    f = ((Math.log(entry.bgValue) ** 0.6678) - 2.767) * 4.608;
    r = 10 * (f ** 2);
    if (f > 0) {
    	srp1_hgbi_total += r;
    }
  ...
  var srp0_hgbi = srp0_hgbi_total / events;
  var srp0_lgbi = srp0_lgbi_total / events;
  var srp1_hgbi = srp1_hgbi_total / events;
  var srp1_lgbi = srp1_lgbi_total / events;
  ...
  if (extra) {
    ...
    $('<tr><td class="tdborder">' + (Math.round(10 * srp0_lgbi) / 10).toFixed(1) + '</td><td class="tdborder">' + (Math.round(10 * srp0_hbgi) / 10).toFixed(1) + '</td><td>' + '</td></tr>').appendTo(stabilitytable);

    $('<tr><th>' + translate('SRP1 LGBI') + '</th><th>' + translate('SRP1 HGBI') + '</th><th>' + '</th></tr>').appendTo(stabilitytable);
    $('<tr><td class="tdborder">' + (Math.round(10 * srp1_lgbi) / 10).toFixed(1) + '</td><td class="tdborder">' + (Math.round(10 * srp1_hbgi) / 10).toFixed(1) + '</td><td>' + '</td></tr>').appendTo(stabilitytable);
  }
