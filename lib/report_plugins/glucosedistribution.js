...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var tar2 = (result.SuperHigh.readingspct + result.High.readingspct + 0.0).toFixed(1);
  var tar2count = !result.SuperHigh.ignore + !result.High.ignore;

  var tar3 = $(result.Elevated.readingspct + tar2 + 0).toFixed(1);
  var tar3count = !result.Elevated.ignore + tar2count;
  var preg3 = $(result.High.readingspct + result.Elevated.readingspct + 0).toFixed(1);
  var preg3count = !noGravid * (!result.Elevated.ignore + !result.High.ignore);

  var tar4 = $(result.Up.readingspct + tar3 + 0).toFixed(1);
  var tar4count = !result.Up.ignore + tar3count;

  var preg1 = $(result.Normal.readingspct + result.Lowish.readingspct + 0).toFixed(1);
  var preg2 = result.Up.readingspct + preg1;

  var titr = $(result.Normal.readingspct + result.Up.readingspct + 0).toFixed(1);
