...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('Ranges') + ':' +
    '<input type="radio" name="rp_ranges" id="rp_customrange" checked>' +
    '<label for="rp_custom" class="translate">Custom</label>' +
    '&nbsp;' +
    '<input type="radio" name="rp_ranges" id="rp_standardrange">' +
    '<label for="rp_standard" class="translate">Standard</label>' +
    '<h2>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var haveStrict = 0;
  var haveBelow = 0;
  var extrarangecols = 0;
  var rangecols = 3;

  var ss = require('simple-statistics');

  var colors = ['#f55', '#f88', '#fb0', '#8f8', '#cf8', '#cf0', '#ff8', '#ff0'];
  var tablecolors = {
    SuperLow: '#f55'
    , Low: '#f88'
    , Lowish: '#fb0'
  ...
  if ($('#rp_customrange').prop('checked', true)) {
    result['SuperLow']  = { }
    result['Low']       = { nextbottom: options.superLow }
    result['Lowish']    = { bottom: options.superLow, nextbottom: options.targetLow }
    result['Normal']    = { bottom: options.targetLow, top: options.targetHigh }
    result['Up']        = { }
    result['Elevated']  = { top: options.superHigh, nexttop: options.targetHigh }
    result['High']      = { nexttop: options.superHigh }
    result['SuperHigh'] = { }
  } else {
    if (displayUnits === 'mmol') {
      result['SuperLow']  = { nextbottom: client.utils.scaleMgdl(54) }
      result['Low']       = { bottom: client.utils.scaleMgdl(54), nextbottom: client.utils.scaleMgdl(63) }
      result['Lowish']    = { bottom: client.utils.scaleMgdl(63), nextbottom: client.utils.scaleMgdl(70) }
      result['Normal']    = { bottom: client.utils.scaleMgdl(70), top: client.utils.scaleMgdl(120) }
      result['Up']        = { top: client.utils.scaleMgdl(140), nexttop: client.utils.scaleMgdl(120) }
      result['Elevated']  = { top: client.utils.scaleMgdl(180), nexttop: client.utils.scaleMgdl(140) }
      result['High']      = { top: client.utils.scaleMgdl(250), nexttop: client.utils.scaleMgdl(180) }
      result['SuperHigh'] = { nexttop: client.utils.scaleMgdl(250) }
    } else {
      result['SuperLow']  = { nextbottom: 54 }
      result['Low']       = { bottom: 54, nextbottom: 63 }
      result['Lowish']    = { bottom: 63, nextbottom: 70 }
      result['Normal']    = { bottom: 70, top: 120 }
      result['Up']        = { top: 140, nexttop: 120 }
      result['Elevated']  = { top: 180, nexttop: 140 }
      result['High']      = { top: 250, nexttop: 180 }
      result['SuperHigh'] = { nexttop: 250 }
    }
  }
  ...
  if (options.strictHigh != options.targetHigh) {
    haveStrict = 1;
  }
  if (options.targetLow != options.superLow) {
    if (!haveAbove) {
      extrarangecols = 1;
      rangecols = 2;
    }
  ...
  $('<th colspan="5">' + translate('% of Readings') + '</th>').appendTo(thead);
  ...
  ['SuperLow', 'Low', 'Lowish', 'Normal', 'Up', 'Elevated', 'High', 'SuperHigh'].forEach(function(range) {
    ...
    r.rangeRecords = glucose_data.filter(function(q) {
      ...
      } else if (typeof r.top != 'undefined' && typeof r.nexttop != 'undefined') {
        return q.sgv > r.nexttop && q.sgv <= r.top;		// elevated
      } else if (typeof r.bottom != 'undefined' && typeof r.nextbottom != 'undefined') {
        return q.sgv >= r.bottom && q.sgv < r.nextbottom;	// lowish
      } else if (typeof r.nexttop != 'undefined') {
        return q.sgv > r.nexttop;				// extreme high
      } else if (typeof r.nextbottom != 'undefined') {
        return q.sgv > 0 && q.sgv < r.nextbottom;		// extreme low
      } else {
      	return 0;
      }
    });
  ...
  var tar2desc = '(>' + options.superHigh + ')';
  var tar3 = result.Elevated.readingspct + tar2;
  var tar3desc = '(>' + options.targetHigh + ')';
  var tar4 = result.Up.readingspct + tar3;
  var tar4desc = '(>' + options.strictHigh + ')';
  ...
  var tbr1desc = '(<' + options.superLow + ')';
  var tbr2 = tbr1 + result.Lowish.readingspct;
  var tbr2desc = '(<' + options.targetLow + ')';
  var tbr3 = result.Low.readingspct + result.Lowish.readingspct;
  var tbr3desc = '(>=' + options.ultraLow + ',&nbsp;<' + options.targetLow + ')';
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'SuperLow') {
      //rangeExp = ' (<' + options.ultraLow + ')';
      rangeLabel = 'Super Low';
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"></td>').appendTo(tr);
    }
    if (range == 'Low') {
    //  rangeExp = ' (>=' + options.ultraLow + ',&nbsp;<' + options.superLow + ')';
      rangeExp = options.ultraLow
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '">&nbsp;<br/><font size="-1">' + options.ultraLow + '</font></td>').appendTo(tr);
    }
    if (range == 'Lowish') {
      //rangeExp = ' (>=' + options.superLow + ',&nbsp;<' + options.targetLow + ')';
      rangeExp = options.superLow
      rangeLabel = 'Lowish';
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '">&nbsp;<br/><font size="-1">' + options.superLow + '</font></td>').appendTo(tr);
    }
    if (range == 'Normal') {
      // rangeExp = ' (>=' + options.targetLow + ',&nbsp;<=' + options.strictHigh + ')';
      //if (options.strictHigh == options.targetHigh) {
        rangeLabel = 'Tight Range';
      //} else {
        //rangeLabel = 'Strict Range';
      //}
      rangeExp = options.targetLow
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"><font size="-1">' + options.strictHigh + '<br/>' + options.targetLow + '</font></td>').appendTo(tr);
    }
    if (range == 'Up') {
      //rangeExp = ' (>' + options.strictHigh + ',&nbsp;<=' + options.targetHigh + ')';
      rangeExp = options.strictHigh
      rangeLabel = 'Elevated';
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"><font size="-1">' + options.targetHigh + '</font><br/>&nbsp;</td>').appendTo(tr);
    }
    if (range == 'Elevated') {
      //rangeExp = ' (>' + options.targetHigh + ',&nbsp;<=' + options.superHigh + ')';
      rangeExp = options.targetHigh
      rangeLabel = 'Above target';
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"><font size="-1">' + options.superHigh + '</font><br/>&nbsp;</td>').appendTo(tr);
    }
    if (range == 'High') {
      //rangeExp = ' (>' + options.superHigh + ',&nbsp;<=' + options.ultraHigh + ')';
      rangeExp = options.superHigh
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"><font size="-1">' + options.ultraHigh + '</font><br/>&nbsp;</td>').appendTo(tr);
    }
    if (range == 'SuperHigh') {
      //rangeExp = ' (>' + options.ultraHigh + ')';
      rangeExp = options.ultraHigh
      rangeLabel = 'Super High';
      $('<td class="tdborder" style="background-color:' + tablecolors[range] + '"></td>').appendTo(tr);
    }
    ...
    if (range == 'SuperHigh') {
      ...
      $('<td class="tdborder" rowspan="2">' + tar2.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" rowspan="3" colspan="2">' + tar3.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" rowspan="4">' + tar4.toFixed(1) + '%</td>').appendTo(tr);
    }
    if (range == 'High') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
    }
    if (range == 'Elevated') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="3">' + translate('TIR') + '<br/><strong>' + tir.toFixed(1) + '%</strong></td>').appendTo(tr);
    } 
    if (range == 'Up') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" style="background-color:' + tablecolors['Up'] + '" rowspan="2">' + translate('TItR') + '<br/><strong>' + titr.toFixed(1) + '%</strong></td>').appendTo(tr);
      $('<td class="tdborder" rowspan="3">' + translate('Preg') + '<br/><strong>' + preg2.toFixed(1) + '%</strong></td>').appendTo(tr);
    } 
    if (range == 'Normal') {
      $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      $('<td class="tdborder" rowspan="2">' + preg1.toFixed(1) + '%</td>').appendTo(tr);
    }
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" rowspan="3"><strong>' + tbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      $('<td class="tdborder" rowspan="2">' + tbr3.toFixed(1) + '%</td>').appendTo(tr);
    }
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td class="tdborder" colspan="2" rowspan="2"><strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
    }
    if (range == 'SuperLow') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      $('<td></td>').appendTo(tr);
    ...
    if ((range == 'Lowish' && !haveBelow) || (range == 'Elevated' && !haveAbove) || (range == 'Up' && !haveStrict)) {
  ...
  if (glucose_data.length > 0) {
    ...
    $('<td class="tdborder">' + (Math.round(ss.standard_deviation(localBgs) * 10) / 10).toFixed(1) + ' (<strong>' + (Math.round(100*(ss.standard_deviation(localBgs)/ss.mean(localBgs)))).toFixed(0) + '%</strong>)</td>').appendTo(tr);
  } else {
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
    $('<td class="tdborder">N/A</td>').appendTo(tr);
  ...
  if (glucose_data.length > 0) {
    ...
    //if (a1cmd === a1cwd && a1cmi === a1cwi) {
    //  $('<td class="tdborder"><center><strong>' + a1cmd + '%</strong><sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub></center></td>').appendTo(predtr);
    //} else {
    //  $('<td class="tdborder"><center>Mean:&nbsp;' + a1cmd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub>' +
    //      '<br />Weighted:&nbsp;' + a1cwd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cwi + '<sub>IFCC</sub></center></td>').appendTo(predtr);
    //}
    //if (gmim === gmiw) {
    //  $('<td class="tdborder"><center>' + gmim + '%</center></td>').appendTo(predtr);
    //} else {
    //  $('<td class="tdborder"><center>Mean:&nbsp;' + gmim + '%<br />Weighted:&nbsp;' + gmiw + '%</center></td>').appendTo(predtr);
    //}
    ...
    if (a1cmd === a1cwd && a1cmi === a1cwi && gmim == gmiw) {
      var predtr = $('<tr>');
      $('<td></td>').appendTo(predtr);
      $('<td class="tdborder"><center><strong>' + a1cwd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cwi + '<sub>IFCC</sub></strong></center></td>').appendTo(predtr);
      $('<td class="tdborder"><center>' + gmiw + '%</center></td>').appendTo(predtr);
      ...
      $('<td class="tdborder"><center>' + a1cmd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub></center></td>').appendTo(predtr);
      $('<td class="tdborder"><center>' + gmim + '%</center></td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
    }
...
//  $('<tr><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td></tr>').appendTo(stabilitytable);
  $('<tr><th>' + translate('Mean Hourly Change') + '</th><th>' + translate('Out of Range RMS') + '</th><th>GVI</th></tr>').appendTo(stabilitytable);
