...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      ...
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (2-custom) + '">' + tar3.toFixed(1) + '%</td>').appendTo(tr);
    ...
    if (range == 'Elevated') {
      ...
      if (tircount > 1) {
        if (!!custom) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="' + tircount + '"><strong>' + tir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+extra) + '" rowspan="' + tbr1count + '"><strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
