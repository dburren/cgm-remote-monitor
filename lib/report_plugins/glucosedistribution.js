...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('here') +
    '</a>.<br/><br/>' +
    'Symmetric Risk Profile (low and high) indices.1</div><br/><br/>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  if (extra) {
    ...
    $('<tr><td class="tdborder">' + (Math.round(10 * lgbi) / 10).toFixed(1) + '</td><td class="tdborder">' + (Math.round(10 * hgbi) / 10).toFixed(1) + '</td><td>' + '</td></tr>').appendTo(stabilitytable);

    hgbi = srp1_hgbi_total / events;
    lgbi = srp1_lgbi_total / events;
