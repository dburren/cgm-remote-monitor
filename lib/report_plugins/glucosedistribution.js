...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  minute_data.push(data[data.length - 1]);
  ...
  var lastMinTime = minute_data[minute_data.length - 1].displayTime.getTime();
  var mintotal = 0;
  var mindivisor = 0.001;
  for (i = 1; i <= minute_data.length - 2; i++) {
    let entry = minute_data[i];
    let days = (lastMinTime - entry.displayTime.getTime()) / (1000.0 * 60 * 60 * 24);
    ...
    if (w > 0) {
      mintotal += w * entry.bgValue;
      mindivisor += w;
    }
  }
  var minweighted = mintotal / mindivisor;
  ...
  ['SuperLow', 'Low', 'Lowish', 'Normal', 'Up', 'Elevated', 'High', 'SuperHigh'].forEach(function(range) {
    let sr = result[range];
    minresult[range] = {};
    minresult[range].ignore = sr.ignore;
    minresult[range].bottom = sr.bottom;
    minresult[range].nextbottom = sr.nextbottom;
    minresult[range].top = sr.top;
    minresult[range].nexttop = sr.nexttop;

    var r = minresult[range];
    r.rangeRecords = minute_data.filter(function(q) {
      if (!!r.ignore) {
        return 0;
      }
    ...
    minstats.push(r.rangeRecords.length);
    r.rangeRecords.sort(function(a, b) {
      return a.sgv - b.sgv;
    });
    r.localBgs = r.rangeRecords.map(function(r) {
  ...
  minresult.Normal.readingspct = (100 - minresult.Low.readingspct - minresult.Lowish.readingspct - minresult.Up.readingspct - minresult.Elevated.readingspct - minresult.High.readingspct - minresult.SuperLow.readingspct - minresult.SuperHigh.readingspct);

  var mintar2 = minresult.SuperHigh.readingspct + minresult.High.readingspct;
  var mintar2count = !minresult.SuperHigh.ignore + !minresult.High.ignore;

  var mintar3 = minresult.Elevated.readingspct + mintar2;
  var mintar3count = !minresult.Elevated.ignore + mintar2count;
  var minpreg3 = minresult.High.readingspct + minresult.Elevated.readingspct;
  var minpreg3count = !noGravid * (!minresult.Elevated.ignore + !minresult.High.ignore);

  var mintar4 = minresult.Up.readingspct + mintar3;
  var mintar4count = !minresult.Up.ignore + mintar3count;

  var minpreg1 = minresult.Normal.readingspct + minresult.Lowish.readingspct;
  var minpreg2 = minresult.Up.readingspct + minpreg1;

  var mintitr = minresult.Normal.readingspct + minresult.Up.readingspct;
  var mintitrcount = !minresult.Normal.ignore + !minresult.Up.ignore;
  var mintir = mintitr + minresult.Elevated.readingspct;
  var mintircount = mintitrcount + !minresult.Elevated.ignore;

  var mintbr1 = minresult.Low.readingspct + minresult.SuperLow.readingspct;
  var mintbr1count = !minresult.Low.ignore + !minresult.SuperLow.ignore;

  var mintbr2 = mintbr1 + minresult.Lowish.readingspct;
  var mintbr2count = !minresult.Lowish.ignore + mintbr1count;

  var mintbr3 = minresult.Low.readingspct + minresult.Lowish.readingspct;
  var mintbr3count = !minresult.Lowish.ignore + !minresult.Low.ignore;

  var minhypoComponent = minresult.SuperLow.readingspct + 0.8 * (minresult.Low.readingspct + minresult.Lowish.readingspct);
  var minhyperComponent = minresult.SuperHigh.readingspct + 0.5 * minresult.High.readingspct;
  var mingri = 3.0 * minhypoComponent + 1.6 * minhyperComponent;
  if (mingri > 100.0) {
    mingri = 100.0;
  }
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'SuperHigh' || range == 'High') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
    }

    if (!!topline) {
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1-custom) + '"><br/>' + tar3.toFixed(1) + '%<br/>' + mintar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!!haveBelow && !noGravid && normalcols == 1) {
        $('<td></td>').appendTo(tr);
      }
      if (tar2count > 1) {
        $('<td class="tdborder" rowspan="' + tar2count + '"><br/>' + tar2.toFixed(1) + '%<br/>' + mintar2.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!custom) {
        $('<td></td>').appendTo(tr);
      }
      ...
      if (!!extra) {
        if (tar4count > 1) {
          $('<td class="tdborder" rowspan="' + tar4count + '">' + tar4.toFixed(1) + '%<br/>' + mintar4.toFixed(1) + '%</td>').appendTo(tr);
        } else {
          $('<td></td>').appendTo(tr);
        }
      }
    ...
    if (range == 'High' && !custom && !noGravid && preg3count > 0) {
      $('<td class="tdborder" rowspan="' + preg3count + '">' + preg3.toFixed(1) + '%<br/>' + minpreg3.toFixed(1) + '%</td>').appendTo(tr);
    }
    ...
    if (range == 'Elevated') {
      ...
      if (tircount > 1) {
	if (elevRange == 2) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="' + tircount + '">' + translate('TITR') + '<br/><strong>' + tir.toFixed(1) + '%<br/>' + mintir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else if (elevRange == 3) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="' + tircount + '">' + translate('TIR') + '<br/><strong>' + tir.toFixed(1) + '%<br/>' + mintir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
	  $('<td class="tdborder" rowspan="' + tircount + '">' + tir.toFixed(1) + '%<br/>' + mintir.toFixed(1) + '%</td>').appendTo(tr);
	}
      } else {
        $('<td></td>').appendTo(tr);
      }
    ...
    if (range == 'Up') {
      ...
      if (titrcount > 1) {
	if (upRange == 2) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Up'] + '" rowspan="' + titrcount + '">' + translate('TITR') + '<br/><strong>' + titr.toFixed(1) + '%<br/>' + mintitr.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else if (upRange == 3) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Up'] + '" rowspan="' + titrcount + '">' + translate('TIR') + '<br/><strong>' + titr.toFixed(1) + '%<br/>' + mintitr.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
	  $('<td class="tdborder" rowspan="' + titrcount + '">' + titr.toFixed(1) + '%<br/>' + mintitr.toFixed(1) + '%</td>').appendTo(tr);
      	}
      } else {
        $('<td></td>').appendTo(tr);
      }
      $('<td class="tdborder" style="background-color:' + tablecolors['Lowish'] + '" rowspan="3">' + translate('TING') + '<br/><strong>' + preg2.toFixed(1) + '%<br/>' + minpreg2.toFixed(1) + '%</strong></td>').appendTo(tr);
    } 

    if (range == 'Normal') {
      if (titrcount > 1) {
        $('<td class="tdborder" colspan="' + normalcols + '">' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      } else if (normalRange == 2) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + normalcols + '">' + translate('TITR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalRange == 3) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + normalcols + '">' + translate('TIR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
        $('<td class="tdborder" colspan="' + normalcols + '"><strong>' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      }
      if (!!extra) {
        $('<td class="tdborder" rowspan="2">' + preg1.toFixed(1) + '%<br/>' + minpreg1.toFixed(1) + '%</td>').appendTo(tr);
      }
      if (!!custom) {
    ...
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr2count > 1) {
        $('<td class="tdborder" rowspan="' + tbr2count + '"><br/><strong>' + tbr2.toFixed(1) + '%<br/>' + mintbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
        $('<td></td>').appendTo(tr);
      }
      if (!custom && tbr3count > 1) {
	if (!noGravid) {
          $('<td class="tdborder" rowspan="' + tbr3count + '">' + tbr3.toFixed(1) + '%<br/>' + mintbr3.toFixed(1) + '%</td>').appendTo(tr);
	} else {
          $('<td></td>').appendTo(tr);
	}
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + minr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+extra) + '" rowspan="' + tbr1count + '"><br/><strong>' + tbr1.toFixed(1) + '%<br/>' + mintbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
    ...
    if (r.rangeRecords.length > 0) {
      $('<td class="tdborder">' + (Math.round(10 * r.mean) / 10).toFixed(1) + '<br />' + (Math.round(10 * minr.mean) / 10).toFixed(1) + '</td>').appendTo(tr);
      if (!noGravid) {
	$('<td class="tdborder">' + (Math.round(10 * r.median) / 10).toFixed(1) + '<br />' + (Math.round(10 * minr.median) / 10).toFixed(1) + '</td>').appendTo(tr);
      }
      $('<td class="tdborder">' + (Math.round(10 * r.stddev) / 10).toFixed(1) + '<br/>' + (Math.round(10 * minr.stddev) / 10).toFixed(1) + '</td>').appendTo(tr);
    } else {
  ...
  var minlocalBgs = minute_data.map(function(r) {
    return r.sgv;
  }).filter(function(bg) {
    return !!bg;
  });
  var minmgDlBgs = minute_data.map(function(r) {
    return r.bgValue;
  }).filter(function(bg) {
    return !!bg;
  });
  ...
  var minper_day = (minute_data.length * 1.0 / days).toFixed(0);
  $('<td class="tdborder">' + glucose_data.length + ' (' + per_day + translate('/day') + ')<br>' + minute_data.length + ' (' + minper_day + translate('/day') + ')</td>').appendTo(tr);

  if (glucose_data.length > 0) {
    $('<td class="tdborder">' + (Math.round(10 * ss.mean(localBgs)) / 10).toFixed(1) + '<br />' + (Math.round(10 * ss.mean(minlocalBgs)) / 10).toFixed(1) + '</td>').appendTo(tr);
    if (!noGravid) {
      $('<td class="tdborder">' + (Math.round(10 * ss.quantile(localBgs, 0.5)) / 10).toFixed(1) + '<br/>' + (Math.round(10 * ss.quantile(minlocalBgs, 0.5)) / 10).toFixed(1) + '</td>').appendTo(tr);
    }
    $('<td class="tdborder">' + (Math.round(ss.standard_deviation(localBgs) * 10) / 10).toFixed(1) + '<br/>(<strong>' + (Math.round(100*(ss.standard_deviation(localBgs)/ss.mean(localBgs)))).toFixed(0) + '%</strong>)<br/>' + (Math.round(ss.standard_deviation(minlocalBgs) * 10) / 10).toFixed(1) + '</td>').appendTo(tr);
  } else {
    $('<td class="tdborder">n/a</td>').appendTo(tr);
    if (!noGravid) {
      $('<td class="tdborder">n/a</td>').appendTo(tr);
  ...
  if (glucose_data.length > 0) {
    ...
    let mina1c_weighted = (minweighted + 46.7) / 28.7;
    var mina1cwd = (Math.round(10 * mina1c_weighted) / 10).toFixed(1);
    if (extra) {
	    mina1cwd = (Math.round(100 * mina1c_weighted) / 100).toFixed(2);
    }

    let mina1cwi = Math.round((mina1c_weighted - 2.15) * 10.929);
    let mingmiwf = minweighted * 0.02392 + 3.31;
    let mingmiw = (Math.round(10*mingmiwf)/10).toFixed(1);
    let mingmiwi = Math.round((mingmiwf * 10.929) - 23.5);

    let mina1c_mean = (ss.mean(minmgDlBgs) + 46.7) / 28.7;
    var mina1cmd = (Math.round(10 * mina1c_mean) / 10).toFixed(1);
    if (extra) {
	    mina1cmd = (Math.round(100 * mina1c_mean) / 100).toFixed(2);
    }
    let mina1cmi = Math.round((mina1c_mean - 2.15) * 10.929);
    let mingmimf = ss.mean(minmgDlBgs) * 0.02392 + 3.31;
    let mingmim = (Math.round(10*mingmimf)/10).toFixed(1);
    let mingmimi = Math.round((mingmimf * 10.929) - 23.5);

    if (!extra || (a1cmd === a1cwd && a1cmi === a1cwi && gmim == gmiw)) {
      var predtr = $('<tr>');
      $('<td></td>').appendTo(predtr);
      $('<td class="tdborderl"><strong>' + a1cmd + '<sub>&nbsp;%</sub>&nbsp;(' + mina1cmd + '<sub>&nbsp;%</sub>) ' + a1cmi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mina1cmi + '<sub>&nbsp;mmol/mol</sub>)</strong></td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmim + '<sub>&nbsp;%</sub>&nbsp;(' + mingmim + '<sub>&nbsp;%</sub>) ' + gmimi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mingmimi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
    } else {
      var predtr = $('<tr>');
      $('<td>' + translate('Mean') + '</td>').appendTo(predtr);
      $('<td class="tdborderl">' + a1cmd + '<sub>&nbsp;%</sub>&nbsp;(' + mina1cmd + '<sub>&nbsp;%</sub>) ' + a1cmi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mina1cmi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmim + '<sub>&nbsp;%</sub>&nbsp;(' + mingmim + '<sub>&nbsp;%</sub>) ' + gmimi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mingmimi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
      ...
      $('<td class="tdborderl"><strong>' + a1cwd + '<sub>&nbsp;%</sub>&nbsp;(' + mina1cwd + '<sub>&nbsp;%</sub>) ' + a1cwi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mina1cwi + '<sub>&nbsp;mmol/mol</sub>)</strong></td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmiw + '<sub>&nbsp;%</sub>&nbsp;(' + mingmiw + '<sub>&nbsp;%</sub>) ' + gmiwi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + mingmiwi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
    }
  } else {
  ...
  var minGVITotal = 0;
  var minGVIIdeal = 0;
  var minGVIIdeal_Time = 0;

  var minRMSTotal = 0;

  var minusedRecords = 0;
  var minglucoseTotal = 0;
  var mindeltaTotal = 0;

  for (i = 0; i <= minute_data.length - 2; i++) {
    const entry = minute_data[i];
    const nextEntry = minute_data[i + 1];
    const timeDelta = nextEntry.displayTime.getTime() - entry.displayTime.getTime();
    ...
    if (delta > 0) { // avoid divide by 0 error
      // Are we rising at faster than 5mg/DL/5minutes
      if ((delta / timeDelta) >= (mint1 / (1000 * 60 * 5))) {
        mint1count += 1;
      }
      // Are we rising at faster than 10mg/DL/5minutes
      if ((delta / timeDelta) >= (mint2 / (1000 * 60 * 5))) {
        mint2count += 1;
      }
    }

    // Calculate the distance travelled for this time step
    minGVITotal += Math.sqrt(Math.pow(timeDelta / (1000 * 60), 2) + Math.pow(delta, 2));

    // Keep track of the number of minutes in this timestep
    minGVIIdeal_Time += timeDelta / (1000 * 60);
    minglucoseTotal += entry.bgValue;

    if (entry.bgValue < options.targetLow) {
      minRMSTotal += Math.pow(options.targetLow - entry.bgValue, 2);
    }
    if (entry.bgValue > options.targetHigh) {
      minRMSTotal += Math.pow(entry.bgValue - options.targetHigh, 2);
    }
  }

  // Difference between first and last reading
  var minGVIDelta = Math.floor(minute_data[0].bgValue - minute_data[minute_data.length - 1].bgValue);

  // Delta for total time considered against total period rise
  minGVIIdeal = Math.sqrt(Math.pow(minGVIIdeal_Time, 2) + Math.pow(minGVIDelta, 2));

  var minGVI = Math.round(minGVITotal / minGVIIdeal * 100) / 100;
  console.log('GVI', minGVI, 'GVIIdeal', minGVIIdeal, 'GVITotal', minGVITotal, 'GVIIdeal_Time', minGVIIdeal_Time);
  ...
  if (!noGravid) {
    ...
    $('<tr><td class="tdborder">' + TDCHourly + unitString + '</td><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td><td class="tdborder">' + GVI + ' (' + minGVI + ')</td></tr>').appendTo(stabilitytable);
  }
