...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('Ranges') + ':' +
    '<input type="radio" name="rp_optionsstddist" id="rp_custom" checked>' +
    '<label for="rp_custom" class="translate">Custom</label>' +
    '&nbsp;' +
    '<input type="radio" name="rp_optionsstddist" id="rp_standard">' +
    '<label for="rp_standard" class="translate">Standard</label><br/>' +
    translate('Display') + ': ' +
    '<h2>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperLow', 'Low', 'Lowish', 'Normal', 'Up', 'Elevated', 'High', 'SuperHigh'].forEach(function(range) {
    ...
    r.rangeRecords = glucose_data.filter(function(r) {
      if (range === 'SuperLow') {
        return r.sgv > 0 && r.sgv < options.ultraLow;
      } else if (range === 'Low') {
        return r.sgv > options.ultraLow && r.sgv < options.superLow;
        ...
        return r.sgv >= options.targetLow && r.sgv <= options.strictHigh;
      } else if (range === 'Up') {
	if (!!haveStrict) {
          return r.sgv > options.strictHigh && r.sgv <= options.targetHigh;
	} else {
	  return 0;
	}
        ...
        return r.sgv > options.superHigh && r.sgv <= options.ultraHigh;
      } else {
        return r.sgv > options.ultraHigh;
      }
    });
