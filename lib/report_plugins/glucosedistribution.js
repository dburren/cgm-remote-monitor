...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<label for="rp_standardrange" class="translate">Standards</label>' +
    '&nbsp;' +
    '<input type="radio" name="rp_ranges" id="rp_extrarange">' +
    '<label for="rp_extrarange" class="translate">Standards+extra</label>' +
    '<h2>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      ...
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="2">3 ' + tar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!haveAbove) {
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+extra) + '" rowspan="' + tbr1count + '">1 <strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
        $('<td colspan="' + haveAbove + '">1</td>').appendTo(tr);
      }
      if (!!extra && !custom) {
        $('<td rowspan="2">6</td>').appendTo(tr);
      }
    }
    if (range == 'SuperLow') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
