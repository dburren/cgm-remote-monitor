...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var tar3 = (Number(result.Elevated.readingspct) + Number(tar2));//.toFixed(1);
  var tar3count = !result.Elevated.ignore + tar2count;
  var preg3 = (Number(result.High.readingspct) + Number(result.Elevated.readingspct));//.toFixed(1);
  var preg3count = !noGravid * (!result.Elevated.ignore + !result.High.ignore);

  var tar4 = (Number(result.Up.readingspct) + Number(tar3));//.toFixed(1);
  var tar4count = !result.Up.ignore + tar3count;

  var preg1 = (Number(result.Normal.readingspct) + Number(result.Lowish.readingspct));//.toFixed(1);
  var preg2 = result.Up.readingspct + preg1;

  var titr = (Number(result.Normal.readingspct) + Number(result.Up.readingspct));//.toFixed(1);
  ...
  var tbr1 = (Number(result.Low.readingspct) + Number(result.SuperLow.readingspct));//.toFixed(1);
  var tbr1count = !result.Low.ignore + !result.SuperLow.ignore;

  var tbr2 = (Number(tbr1) + Number(result.Lowish.readingspct));//.toFixed(1);
  var tbr2count = !result.Lowish.ignore + tbr1count;

  var tbr3 = (Number(result.Low.readingspct) + Number(result.Lowish.readingspct));//.toFixed(1);
