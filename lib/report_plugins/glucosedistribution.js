...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<label for="rp_standardrange" class="translate">Standard</label>' +
    '&nbsp;' +
    '<input type="radio" name="rp_ranges" id="rp_pregnancyrange">' +
    '<label for="rp_pregnancyrange" class="translate">(with pregnancy)</label>' +
    '<h2>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var pregnancy = 0;
  if ($('#rp_pregnancyrange').is(':checked', true)) {
    pregnancy = 1;
    rangecols = 5;
  }
  if (!!custom) {
    rangecols = 1 + haveAbove + haveBelow;
  }
  //var rows = 0;
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    var rangelabel = '';
    if (!!topline) {
      rangelabel = '</font><br/><strong>High</strong><font size="-1">';
    }
    if (range == 'Normal') {
      rangelabel = '</font><br/><strong>In Range</strong><font size="-1">';
    }
    if (range == 'SuperLow' || (range == 'Low' && !!result['SuperLow'].ignore)) {
      rangelabel = '</font><br/><strong>Low</strong><font size="-1">';
    }
    ...
    if (!!topline) {
      if (tar2count > 1) {
        $('<td class="tdborder" rowspan="' + tar2count + '">2 ' + tar2.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!custom) {
        $('<td>2</td>').appendTo(tr);
      }
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1+pregnancy) + '">3 ' + tar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!haveAbove) {
        $('<td colspan="' + 1+ '">3</td>').appendTo(tr);
      }
      if (!!pregnancy) {
        if (tar4count > 1) {
          $('<td class="tdborder" rowspan="' + tar4count + '">4 ' + tar4.toFixed(1) + '%</td>').appendTo(tr);
        } else {
          $('<td>4</td>').appendTo(tr);
        }
      } else if (!!custom && !!haveAbove) {
          $('<td>4a</td>').appendTo(tr);
      }
    }
    if (range == 'Elevated') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
    ...
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr2count > 1) {
        $('<td class="tdborder" rowspan="' + tbr2count + '">2 <strong>' + tbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
        $('<td>2</td>').appendTo(tr);
      }
      if (!!pregnancy && tbr3count > 1) {
        $('<td class="tdborder" rowspan="' + tbr3count + '">3 ' + tbr3.toFixed(1) + '%</td>').appendTo(tr);
      } else {
        $('<td>3</td>').appendTo(tr);
      }
    }
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+pregnancy) + '" rowspan="' + tbr1count + '">1 <strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
        $('<td colspan="' + haveAbove + '">1</td>').appendTo(tr);
      }
    }
    if (range == 'SuperLow') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
