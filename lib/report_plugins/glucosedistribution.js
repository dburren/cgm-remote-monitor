...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      ...
      if ((!!haveAbove || !!haveBelow) && !noGravid && normalcols == 1) {
        $('<td>c</td>').appendTo(tr);
      }
      if (!!extra) {
        if (tar4count > 1) {
    ...
    if (range == 'Normal') {
      if (titrcount > 1) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + normalcols + '">' + translate('TISR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
    ...
    if (range == 'Low') {
      ...
      if (tbr1count > 1) {
        ...
        $('<td colspan="' + (2*haveAbove-haveBelow) + '">l</td>').appendTo(tr);
      }
      //if (!extra && !custom) {
