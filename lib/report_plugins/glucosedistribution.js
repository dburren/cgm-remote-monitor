...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    translate('GVI (Glycemic Variability Index) is a measure developed by Dexcom, detailed <a href="') +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  for (i = 1; i <= glucose_data.length - 2; i++) {
    ...
    // 1.0 for most recent. 0.0 for 120 days ago.

    // Flat
    //let w = 1.0;

    // Linear
    // let w = 1.0 - days/120.0;

    // Log
    let w = 1.0;
    if (days > 0) {
      w = 1.0 - Math.log(days)/log120;
    }
  ...
  if (glucose_data.length > 0) {
    ...
    if (a1cmd === a1cwd && a1cmi === a1cwi) {
      $('<td class="tdborder"><center>' + a1cmd + '%<sub>DCCT</sub>&nbsp;|&nbsp;' + a1cmi + '<sub>IFCC</sub></center></td>').appendTo(tr);
    } else {
...
//  $('<tr><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td></tr>').appendTo(stabilitytable);
  $('<tr><th>' + translate('Mean Hourly Change') + '</th><th>GVI</th><th>Out of Range RMS</th></tr>').appendTo(stabilitytable);
  $('<tr><td class="tdborder">' + TDCHourly + unitString + '</td><td class="tdborder">' + GVI + '</td><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td></tr>').appendTo(stabilitytable);
