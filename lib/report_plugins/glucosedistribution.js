...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var normalRange = 0;
  if (displayUnits === 'mmol') {
    val bottom = result['Normal'].bottom;
    val top = result['Normal'].top;
    if (bottom == client.utils.scaleMgdl(70)) {
      if (top == client.utils.scaleMgdl(120)) {
        normalRange = 1;        // TISR
      } else if (top == client.utils.scaleMgdl(140)) {
        normalRange = 2;        // TITR
      } else if (top == client.utils.scaleMgdl(180)) {
        normalRange = 3;        // TIR
      }
    } else if (bottom == client.utils.scaleMgdl(63) && top == client.utils.scaleMgdl(140)) {
      normalRange = 4;		// TING
    }
  } else {
    val bottom = result['Normal'].bottom;
  ...
  var preg3count = !noTING * (!result.Elevated.ignore + !result.High.ignore);
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      ...
      } else if (!!haveBelow && !noTING && normalcols == 1) {
        $('<td></td>').appendTo(tr);
      }
      if (tar2count > 1) {
    ...
    if (range == 'High' && !custom && !noTING && preg3count > 0) {
