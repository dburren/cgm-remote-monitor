...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    ...
    if (range != 'High') {
      $('<td class="tdborder"><strong>' + r.readingspct + '%</strong></td>').appendTo(tr);
    } else {
  ...
  if (glucose_data.length > 0) {
    ...
    //}
    $('<td class="tdborder"><center><strong>' + a1cwd + '%</strong><sub>DCCT</sub>&nbsp;|&nbsp;' + a1cwi + '<sub>IFCC</sub></center></td>').appendTo(tr);
