...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  result.Normal.readingspct = (100 - result.Low.readingspct - result.Lowish.readingspct - result.Elevated.readingspct - result.High.readingspct);

  var tar = result.Elevated.readingspct + result.High.readingspct;
  var tir = result.Normal.readingspct + result.Elevated.readingspct;
  ...
  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    ...
    if ((range == 'Lowish' && !haveBelow) || (range == 'Elevated' && !haveAbove)) {
  ...
  var per_day = (glucose_data.length * 1.0 / days).toFixed(1);
  $('<td class="tdborder">' + glucose_data.length + ' (' + per_day + ' ' + translate('per day') + ')</td>').appendTo(tr);
