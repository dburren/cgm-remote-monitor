...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      if (tar2count > 1) {
        $('<td class="tdborder" rowspan="' + tar2count + '">' + tar2.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!custom) {
        $('<td></td>').appendTo(tr);
      }
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1+pregnancy) + '">' + tar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!haveAbove) {
