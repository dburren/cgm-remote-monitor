...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var minute_flat_data = [];
  var minute_interp_data = [];
  ...
  for (i = 0; i <= data.length - 2; i++) {
    ...
    if (timeDelta < 3 * 30 * 1000) { // 1.5 minutes
      minute_interp_data.push(entry);
      minute_flat_data.push(entry);
    } else {
      // Don't interpolate extra minutes: just maintain last value!

      var missingMinutes = Math.floor(timeDelta / (60 * 1000)) - 1;
      var minPatch = Math.floor(timeDelta / (missingMinutes + 1));
      var minbgDelta = (nextEntry.bgValue - entry.bgValue) / (missingMinutes + 1);

      minute_interp_data.push(entry);
      minute_flat_data.push(entry);

      for (var j = 1; j <= missingMinutes; j++) {
        var t = new Date(entry.displayTime.getTime() + j * minPatch);
        var flatEntry = {
          sgv: displayUnits === 'mmol' ? entry.bgValue / consts.MMOL_TO_MGDL : entry.bgValue
          , bgValue: entry.bgValue
          , displayTime: t
        };
        minute_flat_data.push(flatEntry);
        var bg = Math.floor(entry.bgValue + minbgDelta * j);
        var interpEntry = {
          sgv: displayUnits === 'mmol' ? bg / consts.MMOL_TO_MGDL : bg
          , bgValue: bg
          , displayTime: t
        };
        minute_interp_data.push(interpEntry);
      }
    }
  ...
  minute_flat_data.push(data[data.length - 1]);
  minute_interp_data.push(data[data.length - 1]);
  ...
  minute_interp_data.sort(function(a, b) {
    return a.displayTime.getTime() - b.displayTime.getTime();
  });
  minute_flat_data.sort(function(a, b) {
    return a.displayTime.getTime() - b.displayTime.getTime();
  });
  ...
  five_data.weighted = wtotal / wdivisor;

  lastEntryTime = minute_flat_data[minute_flat_data.length - 1].displayTime.getTime();
  wtotal = 0;
  wdivisor = 0.001;
  for (i = 1; i <= minute_flat_data.length - 2; i++) {
    let entry = minute_flat_data[i];
    let days = (lastEntryTime - entry.displayTime.getTime()) / (1000.0 * 60 * 60 * 24);
  ...
  flat_data.weighted = wtotal / wdivisor;

  lastEntryTime = minute_interp_data[minute_interp_data.length - 1].displayTime.getTime();
  wtotal = 0;
  wdivisor = 0.001;
  for (i = 1; i <= minute_interp_data.length - 2; i++) {
    let entry = minute_interp_data[i];
    let days = (lastEntryTime - entry.displayTime.getTime()) / (1000.0 * 60 * 60 * 24);
  ...
  interp_data.weighted = wtotal / wdivisor;
  ...
  ['SuperLow', 'Low', 'Lowish', 'Normal', 'Up', 'Elevated', 'High', 'SuperHigh'].forEach(function(range) {
    let sr = result[range];
    flatresult[range] = {};
    flatresult[range].ignore = sr.ignore;
    flatresult[range].bottom = sr.bottom;
    flatresult[range].nextbottom = sr.nextbottom;
    flatresult[range].top = sr.top;
    flatresult[range].nexttop = sr.nexttop;

    interpresult[range] = {};
    interpresult[range].ignore = sr.ignore;
    interpresult[range].bottom = sr.bottom;
    interpresult[range].nextbottom = sr.nextbottom;
    interpresult[range].top = sr.top;
    interpresult[range].nexttop = sr.nexttop;

    var r = flatresult[range];
    r.rangeRecords = minute_flat_data.filter(function(q) {
      if (!!r.ignore) {
        return 0;
      }
    ...
    flatstats.push(r.rangeRecords.length);
    r.rangeRecords.sort(function(a, b) {
      return a.sgv - b.sgv;
    });
    r.localBgs = r.rangeRecords.map(function(r) {
    ...
    var r = interpresult[range];
    r.rangeRecords = minute_interp_data.filter(function(q) {
      if (!!r.ignore) {
        return 0;
      }
    ...
    interpstats.push(r.rangeRecords.length);
    r.rangeRecords.sort(function(a, b) {
      return a.sgv - b.sgv;
    });
    r.localBgs = r.rangeRecords.map(function(r) {
  ...
  flatresult.Normal.readingspct = (100 - flatresult.Low.readingspct - flatresult.Lowish.readingspct - flatresult.Up.readingspct - flatresult.Elevated.readingspct - flatresult.High.readingspct - flatresult.SuperLow.readingspct - flatresult.SuperHigh.readingspct);
  interpresult.Normal.readingspct = (100 - interpresult.Low.readingspct - interpresult.Lowish.readingspct - interpresult.Up.readingspct - interpresult.Elevated.readingspct - interpresult.High.readingspct - interpresult.SuperLow.readingspct - interpresult.SuperHigh.readingspct);

  var flattar2 = flatresult.SuperHigh.readingspct + flatresult.High.readingspct;
  var interptar2 = interpresult.SuperHigh.readingspct + interpresult.High.readingspct;
  var flattar2count = !flatresult.SuperHigh.ignore + !flatresult.High.ignore;
  var interptar2count = !interpresult.SuperHigh.ignore + !interpresult.High.ignore;

  var flattar3 = flatresult.Elevated.readingspct + flattar2;
  var interptar3 = interpresult.Elevated.readingspct + interptar2;
  var flattar3count = !flatresult.Elevated.ignore + flattar2count;
  var interptar3count = !interpresult.Elevated.ignore + interptar2count;
  var flatpreg3 = flatresult.High.readingspct + flatresult.Elevated.readingspct;
  var interppreg3 = interpresult.High.readingspct + interpresult.Elevated.readingspct;
  var flatpreg3count = !noGravid * (!flatresult.Elevated.ignore + !flatresult.High.ignore);
  var interppreg3count = !noGravid * (!interpresult.Elevated.ignore + !interpresult.High.ignore);

  var flattar4 = flatresult.Up.readingspct + flattar3;
  var interptar4 = interpresult.Up.readingspct + interptar3;
  var flattar4count = !flatresult.Up.ignore + flattar3count;
  var interptar4count = !interpresult.Up.ignore + interptar3count;

  var flatpreg1 = flatresult.Normal.readingspct + flatresult.Lowish.readingspct;
  var interppreg1 = interpresult.Normal.readingspct + interpresult.Lowish.readingspct;
  var flatpreg2 = flatresult.Up.readingspct + flatpreg1;
  var interppreg2 = interpresult.Up.readingspct + interppreg1;

  var flattitr = flatresult.Normal.readingspct + flatresult.Up.readingspct;
  var interptitr = interpresult.Normal.readingspct + interpresult.Up.readingspct;
  var flattitrcount = !flatresult.Normal.ignore + !flatresult.Up.ignore;
  var interptitrcount = !interpresult.Normal.ignore + !interpresult.Up.ignore;
  var flattir = flattitr + flatresult.Elevated.readingspct;
  var interptir = interptitr + interpresult.Elevated.readingspct;
  var flattircount = flattitrcount + !flatresult.Elevated.ignore;
  var interptircount = interptitrcount + !interpresult.Elevated.ignore;

  var flattbr1 = flatresult.Low.readingspct + flatresult.SuperLow.readingspct;
  var interptbr1 = interpresult.Low.readingspct + interpresult.SuperLow.readingspct;
  var flattbr1count = !flatresult.Low.ignore + !flatresult.SuperLow.ignore;
  var interptbr1count = !interpresult.Low.ignore + !interpresult.SuperLow.ignore;

  var flattbr2 = flattbr1 + flatresult.Lowish.readingspct;
  var interptbr2 = interptbr1 + interpresult.Lowish.readingspct;
  var flattbr2count = !flatresult.Lowish.ignore + flattbr1count;
  var interptbr2count = !interpresult.Lowish.ignore + interptbr1count;

  var flattbr3 = flatresult.Low.readingspct + flatresult.Lowish.readingspct;
  var interptbr3 = interpresult.Low.readingspct + interpresult.Lowish.readingspct;
  var flattbr3count = !flatresult.Lowish.ignore + !flatresult.Low.ignore;
  var interptbr3count = !interpresult.Lowish.ignore + !interpresult.Low.ignore;

  var flathypoComponent = flatresult.SuperLow.readingspct + 0.8 * (flatresult.Low.readingspct + flatresult.Lowish.readingspct);
  var interphypoComponent = interpresult.SuperLow.readingspct + 0.8 * (interpresult.Low.readingspct + interpresult.Lowish.readingspct);
  var flathyperComponent = flatresult.SuperHigh.readingspct + 0.5 * flatresult.High.readingspct;
  var interphyperComponent = interpresult.SuperHigh.readingspct + 0.5 * interpresult.High.readingspct;
  var flatgri = 3.0 * flathypoComponent + 1.6 * flathyperComponent;
  if (flatgri > 100.0) {
    flatgri = 100.0;
  var interpgri = 3.0 * interphypoComponent + 1.6 * interphyperComponent;
  if (interpgri > 100.0) {
    interpgri = 100.0;
  }
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    var tr = $('<tr>');
    var r = result[range];
    var flatr = flatresult[range];
    var interpr = interpresult[range];
    ...
    if (range == 'SuperHigh' || range == 'High') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
    }

    if (!!topline) {
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1-custom) + '"><br/>' + tar3.toFixed(1) + '%<br/>' + flattar3.toFixed(1) + '% ' + interptar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!!haveBelow && !noGravid && normalcols == 1) {
        $('<td></td>').appendTo(tr);
      }
      if (tar2count > 1) {
        $('<td class="tdborder" rowspan="' + tar2count + '"><br/>' + tar2.toFixed(1) + '%<br/>' + flattar2.toFixed(1) + '% ' + interptar2.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!custom) {
        $('<td></td>').appendTo(tr);
      }
      ...
      if (!!extra) {
        if (tar4count > 1) {
          $('<td class="tdborder" rowspan="' + tar4count + '">' + tar4.toFixed(1) + '%<br/>' + flattar4.toFixed(1) + '% ' + interptar4.toFixed(1) + '%</td>').appendTo(tr);
        } else {
          $('<td></td>').appendTo(tr);
        }
      }
    ...
    if (range == 'High' && !custom && !noGravid && preg3count > 0) {
      $('<td class="tdborder" rowspan="' + preg3count + '">' + preg3.toFixed(1) + '%<br/>' + flatpreg3.toFixed(1) + '% ' + interppreg3.toFixed(1) + '%</td>').appendTo(tr);
    }
    ...
    if (range == 'Elevated') {
      ...
      if (tircount > 1) {
	if (elevRange == 2) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="' + tircount + '">' + translate('TITR') + '<br/><strong>' + tir.toFixed(1) + '%<br/>' + flattir.toFixed(1) + '% ' + interptir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else if (elevRange == 3) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Elevated'] + '" rowspan="' + tircount + '">' + translate('TIR') + '<br/><strong>' + tir.toFixed(1) + '%<br/>' + flattir.toFixed(1) + '% ' + interptir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
	  $('<td class="tdborder" rowspan="' + tircount + '">' + tir.toFixed(1) + '%<br/>' + flattir.toFixed(1) + '% ' + interptir.toFixed(1) + '%</td>').appendTo(tr);
	}
      } else {
        $('<td></td>').appendTo(tr);
      }
    ...
    if (range == 'Up') {
      ...
      if (titrcount > 1) {
	if (upRange == 2) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Up'] + '" rowspan="' + titrcount + '">' + translate('TITR') + '<br/><strong>' + titr.toFixed(1) + '%<br/>' + flattitr.toFixed(1) + '% ' + interptitr.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else if (upRange == 3) {
	  $('<td class="tdborder" style="background-color:' + tablecolors['Up'] + '" rowspan="' + titrcount + '">' + translate('TIR') + '<br/><strong>' + titr.toFixed(1) + '%<br/>' + flattitr.toFixed(1) + '% ' + interptitr.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
	  $('<td class="tdborder" rowspan="' + titrcount + '">' + titr.toFixed(1) + '%<br/>' + flattitr.toFixed(1) + '% ' + interptitr.toFixed(1) + '%</td>').appendTo(tr);
      	}
      } else {
        $('<td></td>').appendTo(tr);
      }
      $('<td class="tdborder" style="background-color:' + tablecolors['Lowish'] + '" rowspan="3">' + translate('TING') + '<br/><strong>' + preg2.toFixed(1) + '%<br/>' + flatpreg2.toFixed(1) + '% ' + interppreg2.toFixed(1) + '%</strong></td>').appendTo(tr);
    } 

    if (range == 'Normal') {
      if (titrcount > 1) {
        $('<td class="tdborder" colspan="' + normalcols + '">' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      } else if (normalRange == 2) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + normalcols + '">' + translate('TITR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalRange == 3) {
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + normalcols + '">' + translate('TIR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
        $('<td class="tdborder" colspan="' + normalcols + '"><strong>' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      }
      if (!!extra) {
        $('<td class="tdborder" rowspan="2">' + preg1.toFixed(1) + '%<br/>' + flatpreg1.toFixed(1) + '% ' + interppreg1.toFixed(1) + '%</td>').appendTo(tr);
      }
      if (!!custom) {
    ...
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr2count > 1) {
        $('<td class="tdborder" rowspan="' + tbr2count + '"><br/><strong>' + tbr2.toFixed(1) + '%<br/>' + flattbr2.toFixed(1) + '% ' + interptbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
        $('<td></td>').appendTo(tr);
      }
      if (!custom && tbr3count > 1) {
	if (!noGravid) {
          $('<td class="tdborder" rowspan="' + tbr3count + '">' + tbr3.toFixed(1) + '%<br/>' + flattbr3.toFixed(1) + '% ' + interptbr3.toFixed(1) + '%</td>').appendTo(tr);
	} else {
          $('<td></td>').appendTo(tr);
	}
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%<br/>' + flatr.readingspct.toFixed(1) + '% ' + interpr.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+extra) + '" rowspan="' + tbr1count + '"><br/><strong>' + tbr1.toFixed(1) + '%<br/>' + flattbr1.toFixed(1) + '% ' + interptbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
    ...
    if (r.rangeRecords.length > 0) {
      $('<td class="tdborder">' + (Math.round(10 * r.mean) / 10).toFixed(1) + '<br />' + (Math.round(10 * flatr.mean) / 10).toFixed(1) + ' ' + (Math.round(10 * interpr.mean) / 10).toFixed(1) + '</td>').appendTo(tr);
      if (!noGravid) {
	$('<td class="tdborder">' + (Math.round(10 * r.median) / 10).toFixed(1) + '<br />' + (Math.round(10 * flatr.median) / 10).toFixed(1) + ' ' + (Math.round(10 * interpr.median) / 10).toFixed(1) + '</td>').appendTo(tr);
      }
      $('<td class="tdborder">' + (Math.round(10 * r.stddev) / 10).toFixed(1) + '<br/>' + (Math.round(10 * flatr.stddev) / 10).toFixed(1) + ' ' + (Math.round(10 * interpr.stddev) / 10).toFixed(1) + '</td>').appendTo(tr);
    } else {
  ...
  var flatlocalBgs = minute_flat_data.map(function(r) {
    return r.sgv;
  }).filter(function(bg) {
    return !!bg;
  });
  var flatmgDlBgs = minute_flat_data.map(function(r) {
    return r.bgValue;
  }).filter(function(bg) {
    return !!bg;
  });
  var interplocalBgs = minute_interp_data.map(function(r) {
    return r.sgv;
  }).filter(function(bg) {
    return !!bg;
  });
  var interpmgDlBgs = minute_interp_data.map(function(r) {
    return r.bgValue;
  }).filter(function(bg) {
    return !!bg;
  });
  ...
  var minper_day = (minute_flat_data.length * 1.0 / days).toFixed(0);
  $('<td class="tdborder">' + glucose_data.length + ' (' + per_day + translate('/day') + ')<br>' + minute_flat_data.length + ' (' + minper_day + translate('/day') + ')</td>').appendTo(tr);

  if (glucose_data.length > 0) {
    $('<td class="tdborder">' + (Math.round(10 * ss.mean(localBgs)) / 10).toFixed(1) + '<br />' + (Math.round(10 * ss.mean(flatlocalBgs)) / 10).toFixed(1) + ' ' + (Math.round(10 * ss.mean(interplocalBgs)) / 10).toFixed(1) + '</td>').appendTo(tr);
    if (!noGravid) {
      $('<td class="tdborder">' + (Math.round(10 * ss.quantile(localBgs, 0.5)) / 10).toFixed(1) + '<br/>' + (Math.round(10 * ss.quantile(flatlocalBgs, 0.5)) / 10).toFixed(1) + ' ' + (Math.round(10 * ss.quantile(interplocalBgs, 0.5)) / 10).toFixed(1) + '</td>').appendTo(tr);
    }
    $('<td class="tdborder">' + (Math.round(ss.standard_deviation(localBgs) * 10) / 10).toFixed(1) + '<br/>(<strong>' + (Math.round(100*(ss.standard_deviation(localBgs)/ss.mean(localBgs)))).toFixed(0) + '%</strong>)<br/>' + (Math.round(ss.standard_deviation(flatlocalBgs) * 10) / 10).toFixed(1) + ' ' + (Math.round(ss.standard_deviation(interplocalBgs) * 10) / 10).toFixed(1) + '</td>').appendTo(tr);
  } else {
    $('<td class="tdborder">n/a</td>').appendTo(tr);
    if (!noGravid) {
      $('<td class="tdborder">n/a</td>').appendTo(tr);
  ...
  if (glucose_data.length > 0) {
    let a1c_weighted = (five_data.weighted + 46.7) / 28.7;
    ...
    let gmiwf = five_data.weighted * 0.02392 + 3.31;
    ...
    let flata1c_weighted = (flat_data.weighted + 46.7) / 28.7;
    var flata1cwd = (Math.round(10 * flata1c_weighted) / 10).toFixed(1);
    if (extra) {
	    flata1cwd = (Math.round(100 * flata1c_weighted) / 100).toFixed(2);
    }

    let flata1cwi = Math.round((flata1c_weighted - 2.15) * 10.929);
    let flatgmiwf = flat_data.weighted * 0.02392 + 3.31;
    let flatgmiw = (Math.round(10*flatgmiwf)/10).toFixed(1);
    let flatgmiwi = Math.round((flatgmiwf * 10.929) - 23.5);

    let flata1c_mean = (ss.mean(flatmgDlBgs) + 46.7) / 28.7;
    var flata1cmd = (Math.round(10 * flata1c_mean) / 10).toFixed(1);
    if (extra) {
	    flata1cmd = (Math.round(100 * flata1c_mean) / 100).toFixed(2);
    }
    let flata1cmi = Math.round((flata1c_mean - 2.15) * 10.929);
    let flatgmimf = ss.mean(flatmgDlBgs) * 0.02392 + 3.31;
    let flatgmim = (Math.round(10*flatgmimf)/10).toFixed(1);
    let flatgmimi = Math.round((flatgmimf * 10.929) - 23.5);

    let interpa1c_weighted = (interp_data.weighted + 46.7) / 28.7;
    var interpa1cwd = (Math.round(10 * interpa1c_weighted) / 10).toFixed(1);
    if (extra) {
	    interpa1cwd = (Math.round(100 * interpa1c_weighted) / 100).toFixed(2);
    }

    let interpa1cwi = Math.round((interpa1c_weighted - 2.15) * 10.929);
    let interpgmiwf = interp_data.weighted * 0.02392 + 3.31;
    let interpgmiw = (Math.round(10*interpgmiwf)/10).toFixed(1);
    let interpgmiwi = Math.round((interpgmiwf * 10.929) - 23.5);

    let interpa1c_mean = (ss.mean(interpmgDlBgs) + 46.7) / 28.7;
    var interpa1cmd = (Math.round(10 * interpa1c_mean) / 10).toFixed(1);
    if (extra) {
	    interpa1cmd = (Math.round(100 * interpa1c_mean) / 100).toFixed(2);
    }
    let interpa1cmi = Math.round((interpa1c_mean - 2.15) * 10.929);
    let interpgmimf = ss.mean(interpmgDlBgs) * 0.02392 + 3.31;
    let interpgmim = (Math.round(10*interpgmimf)/10).toFixed(1);
    let interpgmimi = Math.round((interpgmimf * 10.929) - 23.5);

    if (!extra || (a1cmd === a1cwd && a1cmi === a1cwi && gmim == gmiw)) {
      var predtr = $('<tr>');
      $('<td></td>').appendTo(predtr);
      $('<td class="tdborderl"><strong>' + a1cmd + '<sub>&nbsp;%</sub>&nbsp;(' + flata1cmd + '<sub>&nbsp;%</sub>) ' + a1cmi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flata1cmi + '<sub>&nbsp;mmol/mol</sub>)</strong></td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmim + '<sub>&nbsp;%</sub>&nbsp;(' + flatgmim + '<sub>&nbsp;%</sub>) ' + gmimi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flatgmimi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
    } else {
      var predtr = $('<tr>');
      $('<td>' + translate('Mean') + '</td>').appendTo(predtr);
      $('<td class="tdborderl">' + a1cmd + '<sub>&nbsp;%</sub>&nbsp;(' + flata1cmd + '<sub>&nbsp;%</sub>) ' + a1cmi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flata1cmi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmim + '<sub>&nbsp;%</sub>&nbsp;(' + flatgmim + '<sub>&nbsp;%</sub>) ' + gmimi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flatgmimi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
      ...
      $('<td class="tdborderl"><strong>' + a1cwd + '<sub>&nbsp;%</sub>&nbsp;(' + flata1cwd + '<sub>&nbsp;%</sub>) ' + a1cwi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flata1cwi + '<sub>&nbsp;mmol/mol</sub>)</strong></td>').appendTo(predtr);
      $('<td class="tdborderl">' + gmiw + '<sub>&nbsp;%</sub>&nbsp;(' + flatgmiw + '<sub>&nbsp;%</sub>) ' + gmiwi + '<sub>&nbsp;mmol/mol</sub>&nbsp;(' + flatgmiwi + '<sub>&nbsp;mmol/mol</sub>)</td>').appendTo(predtr);
      predtable.append(predtr);
      preds.append(predtable);
    }
  } else {
  ...
  var flatGVITotal = 0;
  var flatGVIIdeal = 0;
  var flatGVIIdeal_Time = 0;

  var flatRMSTotal = 0;

  var flatusedRecords = 0;
  var flatglucoseTotal = 0;
  var flatdeltaTotal = 0;

  for (i = 0; i <= minute_flat_data.length - 2; i++) {
    const entry = minute_flat_data[i];
    const nextEntry = minute_flat_data[i + 1];
    const timeDelta = nextEntry.displayTime.getTime() - entry.displayTime.getTime();
    ...
    var delta = Math.abs(nextEntry.bgValue - entry.bgValue);
    flatdeltaTotal += delta;

    if (delta > 0) { // avoid divide by 0 error
      // Are we rising at faster than 5mg/DL/5minutes
      if ((delta / timeDelta) >= (flatt1 / (1000 * 60 * 5))) {
        flatt1count += 1;
      }
      // Are we rising at faster than 10mg/DL/5minutes
      if ((delta / timeDelta) >= (flatt2 / (1000 * 60 * 5))) {
        flatt2count += 1;
      }
    }

    // Calculate the distance travelled for this time step
    flatGVITotal += Math.sqrt(Math.pow(timeDelta / (1000 * 60), 2) + Math.pow(delta, 2));

    // Keep track of the number of minutes in this timestep
    flatGVIIdeal_Time += timeDelta / (1000 * 60);
    flatglucoseTotal += entry.bgValue;

    if (entry.bgValue < options.targetLow) {
      flatRMSTotal += Math.pow(options.targetLow - entry.bgValue, 2);
    }
    if (entry.bgValue > options.targetHigh) {
      flatRMSTotal += Math.pow(entry.bgValue - options.targetHigh, 2);
    }
  }

  // Difference between first and last reading
  var flatGVIDelta = Math.floor(minute_flat_data[0].bgValue - minute_flat_data[minute_flat_data.length - 1].bgValue);

  // Delta for total time considered against total period rise
  flatGVIIdeal = Math.sqrt(Math.pow(flatGVIIdeal_Time, 2) + Math.pow(flatGVIDelta, 2));

  var flatGVI = Math.round(flatGVITotal / flatGVIIdeal * 100) / 100;
  console.log('GVI', flatGVI, 'GVIIdeal', flatGVIIdeal, 'GVITotal', flatGVITotal, 'GVIIdeal_Time', flatGVIIdeal_Time);
  ...
  var interpGVITotal = 0;
  var interpGVIIdeal = 0;
  var interpGVIIdeal_Time = 0;

  var interpRMSTotal = 0;

  var interpusedRecords = 0;
  var interpglucoseTotal = 0;
  var interpdeltaTotal = 0;

  for (i = 0; i <= minute_interp_data.length - 2; i++) {
    const entry = minute_interp_data[i];
    const nextEntry = minute_interp_data[i + 1];
    const timeDelta = nextEntry.displayTime.getTime() - entry.displayTime.getTime();
    ...
    var delta = Math.abs(nextEntry.bgValue - entry.bgValue);
    interpdeltaTotal += delta;

    if (delta > 0) { // avoid divide by 0 error
      // Are we rising at faster than 5mg/DL/5minutes
      if ((delta / timeDelta) >= (interpt1 / (1000 * 60 * 5))) {
        interpt1count += 1;
      }
      // Are we rising at faster than 10mg/DL/5minutes
      if ((delta / timeDelta) >= (interpt2 / (1000 * 60 * 5))) {
        interpt2count += 1;
      }
    }

    // Calculate the distance travelled for this time step
    interpGVITotal += Math.sqrt(Math.pow(timeDelta / (1000 * 60), 2) + Math.pow(delta, 2));

    // Keep track of the number of minutes in this timestep
    interpGVIIdeal_Time += timeDelta / (1000 * 60);
    interpglucoseTotal += entry.bgValue;

    if (entry.bgValue < options.targetLow) {
      interpRMSTotal += Math.pow(options.targetLow - entry.bgValue, 2);
    }
    if (entry.bgValue > options.targetHigh) {
      interpRMSTotal += Math.pow(entry.bgValue - options.targetHigh, 2);
    }
  }

  // Difference between first and last reading
  var interpGVIDelta = Math.floor(minute_interp_data[0].bgValue - minute_interp_data[minute_interp_data.length - 1].bgValue);

  // Delta for total time considered against total period rise
  interpGVIIdeal = Math.sqrt(Math.pow(interpGVIIdeal_Time, 2) + Math.pow(interpGVIDelta, 2));

  var interpGVI = Math.round(interpGVITotal / interpGVIIdeal * 100) / 100;
  console.log('GVI', interpGVI, 'GVIIdeal', interpGVIIdeal, 'GVITotal', interpGVITotal, 'GVIIdeal_Time', interpGVIIdeal_Time);
  ...
  if (!noGravid) {
    ...
    $('<tr><td class="tdborder">' + TDCHourly + unitString + '</td><td class="tdborder">' + Math.round(RMS * 100) / 100 + unitString + '</td><td class="tdborder">' + GVI + ' (' + flatGVI + ' ' + interpGVI + ')</td></tr>').appendTo(stabilitytable);
  }
