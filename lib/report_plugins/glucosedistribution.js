...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr2count > 1) {
        $('<td class="tdborder" rowspan="' + tbr2count + '"><strong>' + tbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
        $('<td>2</td>').appendTo(tr);
      }
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+pregnancy) + '" rowspan="' + tbr1count + '"><strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
