...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (!!topline) {
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1-custom) + '">tar3<br/>' + tar3.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!!haveBelow && !noGravid) {
        $('<td></td>').appendTo(tr);
      }
      if (tar2count > 1) {
        $('<td class="tdborder" rowspan="' + tar2count + '">tar2<br/>' + tar2.toFixed(1) + '%</td>').appendTo(tr);
      } else if (!custom) {
        $('<td></td>').appendTo(tr);
      }
    ...
    if (range == 'Normal') {
      if (titrcount > 1) {
        ...
        $('<td class="tdborder" style="background-color:' + tablecolors['Normal'] + '" colspan="' + titrwidth + '">' + translate('TITR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      ...
      if (!!custom) {
        if ((!!haveAbove || !!haveBelow) && titrwidth == 1) {
          $('<td></td>').appendTo(tr);
        }
      } else {
	if (!!noGravid) {
    ...
    if (range == 'Lowish') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr2count > 1) {
        $('<td class="tdborder" rowspan="' + tbr2count + '">tbr2<br/><strong>' + tbr2.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
        $('<td></td>').appendTo(tr);
      }
      if (!custom && tbr3count > 1) {
	if (!noGravid) {
          $('<td class="tdborder" rowspan="' + tbr3count + '">tbr3<br/>' + tbr3.toFixed(1) + '%</td>').appendTo(tr);
	} else {
          $('<td></td>').appendTo(tr);
	}
    ...
    if (range == 'Low') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '%</td>').appendTo(tr);
      if (tbr1count > 1) {
        $('<td class="tdborder" colspan="' + (1+extra) + '" rowspan="' + tbr1count + '">tbr1<br/><strong>' + tbr1.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (!!haveAbove) {
