...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  $('<th colspan="2">' + translate('% of Readings') + '</th>').appendTo(thead);
  ...
  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    ...
    if (range == 'Elevated') {
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '% (<strong>' + tir.toFixed(1) + '%</strong>)</td>').appendTo(tr);
    } 
    if (range == 'Normal') {
    ...
    if (range == 'Lowish') {
      ...
      if (options.superLow != options.targetLow) {
        ...
        $('<td class="tdborder">' + tbr.toFixed(1) + '%</td>').appendTo(tr);
      }
    }
    if (range == 'Low') {
      $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
