...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '&nbsp;' +
    '<input type="radio" name="rp_ranges" id="rp_nogravid">' +
    '<label for="rp_extrarange" class="translate">Standards-gravid</label>' +
    '<h2>' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var normalName = 0;
  if (displayUnits === 'mmol') {
    if (result['Normal'].bottom == 3.9) {
      if (result['Normal'].top == 6.7 ) {
      	normalName = 1;	// TISR
      } else if (result['Normal'].top == 7.8 ) {
      	normalName = 2;	// TITR
      } else if (result['Normal'].top == 10 ) {
      	normalName = 3;	// TIR
      }
    }
  } else {
  ...
  if (displayUnits === 'mmol') {
    if (result['Normal'].bottom == client.utils.scaleMgdl(70)) {
      if (result['Up'].top == client.utils.scaleMgdl(180)) {
      	upName = 2;	// TITR
  ...
  ['SuperHigh', 'High', 'Elevated', 'Up', 'Normal', 'Lowish', 'Low', 'SuperLow'].forEach(function(range) {
    ...
    if (range == 'Normal') {
      if (!!custom) {
        rangelabel = '</font><br/><strong>' + translate('In Range') + '</strong><font size="-1">' + normalRange + '<br/>';
      } else {
        rangelabel = '<br/>' + normalRange + '<br/>';
      }
    }
    ...
    if (!!topline) {
      ...
      if (tar3count > 1) {
        $('<td class="tdborder" rowspan="' + tar3count + '" colspan="' + (1-custom) + '">' + tar3.toFixed(1) + '%</td>').appendTo(tr);
	//if (!custom && !noGravid) {
        //  $('<td>a</td>').appendTo(tr);
	//}
      } else if (!!haveBelow && !noGravid) {
        $('<td></td>').appendTo(tr);
      }
    ...
    if (range == 'Elevated') {
      ...
      if (tircount > 1) {
        if (!!custom) {
	  $('<td class="tdborder" rowspan="' + tircount + '"><strong>' + tir.toFixed(1) + '%</strong></td>').appendTo(tr);
	} else {
	...
	if (upName == 2) {
      ...
      $('<td class="tdborder" rowspan="3">' + translate('Gravid') + '<br/>' + preg2.toFixed(1) + '%</td>').appendTo(tr);
    } 

    if (range == 'Normal') {
      if (normalName == 1) {
        $('<td class="tdborder">' + translate('TISR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalName == 2) {
        $('<td class="tdborder">' + translate('TITR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else if (normalName == 3) {
        $('<td class="tdborder">' + translate('TIR') + '<br/><strong>' + r.readingspct.toFixed(1) + '%</strong></td>').appendTo(tr);
      } else {
        $('<td class="tdborder"><strong>' + r.readingspct.toFixed(1) + '% ' + normalName + '</strong></td>').appendTo(tr);
      }
      if (!!extra) {
  ...
  if (glucose_data.length > 0) {
    ...
    if (!!noGravid || (a1cmd === a1cwd && a1cmi === a1cwi && gmim == gmiw)) {
      var predtr = $('<tr>');
  ...
  if (!custom) {
    ...
    $('<tr><td class="tdborder">' + Math.round(gri) + '</td><td class="tdborder">' + Math.round(hypoComponent) + '%</td><td class="tdborder">' + Math.round(hyperComponent) + '%</td></tr>').appendTo(stabilitytable);
  }
