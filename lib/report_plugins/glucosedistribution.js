...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  if ($('#rp_customrange').is(':checked', true)) {
    ...
    result['Low']        = { nextbottom: options.superLow, ignore: 0 }
    if (options.superLow != options.targetLow) {
      result['Lowish']   = { bottom: options.superLow, nextbottom: options.targetLow, ignore: 0 }
    } else {
      result['Lowish']   = { ignore: 1 }
    }
    result['Normal']     = { bottom: options.targetLow, top: options.targetHigh, ignore: 0 }
    result['Up']         = { ignore: 1 }
    if (options.superHigh != options.targetHigh) {
      result['Elevated'] = { top: options.superHigh, nexttop: options.targetHigh, ignore: 0 }
    } else {
      result['Elevated'] = { ignore: 1 }
    }
    result['High']       = { nexttop: options.superHigh, ignore: 0 }
  ...
  if (result['Elevated'].ignore && result['Lowish'].ignore) {
    rangecols = 1;
  }
