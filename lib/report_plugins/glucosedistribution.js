...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  result.Normal.readingspct = (100 - result.Low.readingspct - result.Lowish.readingspct - result.Elevated.readingspct - result.High.readingspct).toFixed(1);
  ...
  ['High', 'Elevated', 'Normal', 'Lowish', 'Low'].forEach(function(range) {
    ...
    if (range == 'Elevated') {
      var other = (result['Normal'].readingspct + r.readingspct).toFixed(1);
      $('<td class="tdborder">' + r.readingspct.toFixed(1) + '% (<strong>' + other + '%</strong>)</td>').appendTo(tr);
    } 
    ...
    if (range == 'Low') {
      if (options.superlow == options.targetlow) {
        ...
        var other = (result['Lowish'].readingspct + r.readingspct).toFixed(1);
        $('<td class="tdborder">' + r.readingspct.toFixed(1) + '% (<strong>' + other + '%</strong>)</td>').appendTo(tr);
      } 
    } 
